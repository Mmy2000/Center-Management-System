...
- **Every `<select>` is upgraded in place** by `static/js/select.js`: a styled trigger, a floating panel, a search box once the list passes eight options (type-ahead reveals it below that), keyboard `↑ ↓ Home End Enter Esc`, and `role="combobox"`/`listbox`/`option`. The native element stays in the DOM and stays authoritative — pages keep reading `element.value`, keep listening for `change`, and forms keep posting the field — so a screen that never heard of it works unchanged, and so does the page if JS fails. Search folds Arabic the way a typist types it: `احمد` finds `أحمد`, `فاطمه` finds `فاطمة`, `على` finds `علي`. Opt out with `data-no-enhance`; `multiple` is left native. Modal fields are enhanced by `app.openModal`, and the panel is parented to the modal so Bootstrap's focus trap does not fight the search box.
