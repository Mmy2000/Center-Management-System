...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--check-only", action="store_true", help="Report drift, change nothing.")

    def handle(self, *args, **options):
        result = recalculate_charges(options["month"], fix=not options["check_only"])
        if result["drift"]:
            self.stdout.write(
                self.style.WARNING(f"drift on {len(result['drift'])} charge(s): {result['drift'][:5]}")
            )
